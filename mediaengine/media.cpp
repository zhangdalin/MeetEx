...
MicSource::MicSource(int sample_rate, int channels, int frame_samples,
                     AudioCallback cb)
...
void MicSource::pause() {
    if (stream_) {
        SDL_PauseAudioStreamDevice(stream_); //
    }
}

void MicSource::resume() {
    if (stream_) {
...
SpeakerSink::SpeakerSink(int sample_rate, int channels)
    : sample_rate_(sample_rate), channels_(channels) {}
...
void SpeakerSink::enqueue(const int16_t *samples,
...
void SpeakerSink::pause() {
    if (stream_) {
        SDL_PauseAudioStreamDevice(stream_);
    }
}

void SpeakerSink::resume() {
    if (stream_) {
        SDL_ResumeAudioStreamDevice(stream_);
    }
}
...
CamSource::CamSource(int desired_width, int desired_height,
