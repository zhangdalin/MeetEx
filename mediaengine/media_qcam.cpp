...
bool QCamSource::init()
{
    ...
    if (cameras.isEmpty()) {
        qWarning() << __FUNCTION__ << "No camera devices found";
        return false;
    }

    // Pick the first available camera
    const QCameraDevice &device = cameras.first();
    qInfo() << __FUNCTION__ << "Opening camera:" << device.description();
    ...
    if (!camera_->isActive()) {
        qCritical() << __FUNCTION__ << "Failed to start camera";
        stop();
        return false;
    }

    qInfo() << __FUNCTION__ << "Camera started:" << device.description();
    return true;
}
...
void QCamSource::onVideoFrameReceived(const QVideoFrame &frame)
{
    ...
    if (img.isNull()) {
        qWarning() << __FUNCTION__ << "Failed to convert frame to RGBA8888";
        return;
    }
